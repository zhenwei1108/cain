package com.github.wegoo.cain.crypto.digests;
...
import com.github.wegoo.cain.jcajce.provider.asymmetric.ec.BCECPublicKey;
 ...
 * https://tools.ietf.org/html/draft-shen-sm3-hash-01 and at .... ( Chinese PDF )
 * <p>
 * The specification says "process a bit stream", but this is written to process bytes in blocks of
...
public class SM3Digest
  ...
  private int xOff;
  //double 1~8
  public static final byte[] DEFAULT_USER_ID = Hex.decode("31323334353637383132333435363738");
  ...
  public SM3Digest(BCECPublicKey bcEcPublicKey) {
    this(CryptoServicePurpose.ANY);
    ...
    byte[] gX = g.getAffineXCoord().getEncoded();
    byte[] gY = g.getAffineYCoord().getEncoded();
    byte[] keyX = bcEcPublicKey.getQ().getAffineXCoord().getEncoded();
    byte[] keyY = bcEcPublicKey.getQ().getAffineYCoord().getEncoded();
    int len = DEFAULT_USER_ID.length * 8;
    byte[] fix = new byte[]{(byte) (len >>> 8), (byte) len};
    this.z = Arrays.concatenate(fix,
        Arrays.concatenate(DEFAULT_USER_ID, a, b, Arrays.concatenate(gX, gY, keyX, keyY)));
  }
